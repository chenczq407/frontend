...
**Goal:** Add a required registration avatar flow that supports WeChat avatar selection and album/camera upload, while preserving a clean upload adapter boundary for a future backend service.
...
Create the avatar field UI, wire it to a dedicated avatar composable, and emit complete registration payloads with avatar fields included.
...
Use a Mini Program `button` with `open-type="chooseAvatar"`, read `detail.avatarUrl`, call `uni.chooseImage` for album/camera flows, and centralize upload behavior in `uploadAvatar(filePath)`.
